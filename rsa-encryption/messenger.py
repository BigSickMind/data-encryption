...
def prepared_list(data):
    ...
    msg_recv = msg_crc[0]
    crc_recv = int(msg_crc[1])
    from zlib import crc32
    crc = crc32(msg_recv.encode())
    if crc_recv != crc:
...
def wtite_msg_to_user(from_msg, to_msg, msg, date, time):
    ...
    msg_id = read_msg_id()
    from zlib import crc32
    crc = crc32(crypted_message.encode())
    crypted_message += "c:::"
    crypted_message += str(crc)
    mas = [str(msg_id), from_msg, to_msg, crypted_message, date, time, '0']
    code = making_code(mas, len(mas))
